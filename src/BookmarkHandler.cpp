    ...
    , _views(QList<ViewProperties *>())
{
    ...
    _file = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("konsole-plus/bookmarks.xml"));

    if (_file.isEmpty()) {
        _file = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QStringLiteral("/konsole-plus");
        QDir().mkpath(_file);
