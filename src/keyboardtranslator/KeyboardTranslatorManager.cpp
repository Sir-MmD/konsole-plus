...
{
    auto foundFiles = QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, QStringLiteral("konsole-plus/") + name + QStringLiteral(".keytab")).count();
    if (QFile::exists(QStringLiteral(":/konsole-plus/keyboard-layouts/") + name + QStringLiteral(".keytab"))) {
        ++foundFiles;
    }
    return (foundFiles > 1);
}
...
{
    auto file = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("konsole-plus/") + name + QStringLiteral(".keytab"));

    // fallback to bundled ones
    if (file.isEmpty() && QFile::exists(QStringLiteral(":/konsole-plus/keyboard-layouts/") + name + QStringLiteral(".keytab"))) {
        file = QStringLiteral(":/konsole-plus/keyboard-layouts/") + name + QStringLiteral(".keytab");
    }
...
{
    QStringList dirs = QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, QStringLiteral("konsole-plus"), QStandardPaths::LocateDirectory);
    dirs.append(QStringLiteral(":/konsole-plus/keyboard-layouts")); // fallback to bundled ones
...
{
    const QString dir = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QStringLiteral("/konsole-plus/");
    QDir().mkpath(dir);
